...
CFR_per_age_group = deaths_age_adjusted/cases_age_adjusted
...
multiplier_CFR_comorbid = (0.59*0.8329)/(0.5171) / ((0.59*0.1671)/(0.4829))
#factor_cfr_comorbid = 30
...
prevalence_comorbidities = np.zeros(4)
dummy = n_a
for i in range(18):
    prod_population_comor[i] = dummy[i]*comor_prop_by_by_18agegp[i]
prevalence_comorbidities[0]= (np.sum(prod_population_comor[0:3])+ prod_population_comor[3]*1/5)/(population_by_age_gp_model[0]+n_a[3]*1/5)
prevalence_comorbidities[1]= (prod_population_comor[3]*4/5 + np.sum(prod_population_comor[4:13]))/(n_a[3]*4/5+sum(n_a[4:13]))
prevalence_comorbidities[2]= np.sum(prod_population_comor[13:15])/population_by_age_gp_model[2]
prevalence_comorbidities[3]= np.sum(prod_population_comor[15:])/population_by_age_gp_model[3]    
...
# Nsize = np.zeros(18,dtype=np.float64)     
# gp9_in_agegp = np.array([0,1,1,1,1,2,2,3,3])
# Nsize[0] = Nsize_gps_ires_comor[0]
# Nsize[1] = Nsize_gps_ires_comor[0] 
# for i in range(1,9):
#     Nsize[2*i] = Nsize_gps_ires_comor[i] * (1-prevalence_comorbidities[gp9_in_agegp[i]]) 
#     Nsize[2*i+1] = Nsize_gps_ires_comor[i] * (prevalence_comorbidities[gp9_in_agegp[i]])


# CFR_nocomorbid_4agegp = [age_cfr[i]/(prevalence_comorbidities[i]*factor_cfr_comorbid+(1-prevalence_comorbidities[i])) for i in range(4)]
# CFR = [age_cfr[0], age_cfr[0]]
# for i in range(1,9):
